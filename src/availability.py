...
def compute_multipliers(candidate: dict, jd: dict) -> dict[str, float]:
    """Computes the availability and location multipliers for a candidate."""
    # --- Availability Multiplier ---
    availability_mult = 1.0
    
    # open_to_work_flag
    if candidate.get("open_to_work_flag") is True:
    ...
    salary_range = candidate.get("expected_salary_range_inr_lpa") or {}
    salary_min = 0.0
    ...
    cand_loc = str(candidate.get("location") or "").lower().strip()
